/* g++ final.cpp -o final `pkg-config --libs opencv` -larmadillo -lwiringPi */
#include <iostream>
#include <armadillo>
#include <string>
...
*/      
		...
		if(soln(0, 0) > 0 && soln(0, 0) < image.cols && soln(1, 0) > 0 && soln(1, 0) < image.rows) {
            ...
            if(soln(0, 0) < 240) {
                // 좌회전 연산
                delta = ((240 - soln(0, 0)) / 240) * 120 * ((130 - soln(1, 0)) / 65);
            }/*
            else if(soln(0, 0) >= 230 && soln(0, 0) < 250) {
                // 직진
                delta = 0;
            }*/
            else if(soln(0, 0) >= 240) {
                // 우회전 연산
                delta = -((240 - soln(0, 0)) / 240) * 120 * ((130 - soln(1, 0)) / 65);
            }
            // toDo: use previous frame's result to reduce calculations and stabilize the region of vanishing point
            ...
            if(prevRes(0, 0) < 240) {
                // 좌회전 연산
                delta = ((240 - prevRes(0, 0)) / 240) * 120 * ((130 - prevRes(1, 0)) / 65);
            }/*
            else if(prevRes(0, 0) >= 230 && prevRes(0, 0) < 250) {
                // 직진
                delta = 0;
            }*/
            else if(prevRes(0, 0) >= 240) {
                // 우회전 연산
                delta = -((240 - prevRes(0, 0)) / 240) * 120 * ((130 - prevRes(1, 0)) / 65);
            }
        }
