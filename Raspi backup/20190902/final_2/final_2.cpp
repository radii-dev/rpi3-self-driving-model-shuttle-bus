/*g++ main.c -o main -lwiringPi $(pkg-config --libs --cflags opencv) */
...
int main()
{
	...
	while(true)	{
		...
		/* 노란색 검출 */
		detectHScolor(Video, 25, 45, 30, 255, mask);	// Hue range (20~70), Saturation (15, 230)
		...
		if(vetor_real_of_real > 3700)		vector_real = 3700;
		else if(vetor_real_of_real < 2300)	vector_real = 2300;
		else								vector_real = vetor_real_of_real;

		//imshow("Video", gray);
		
		buffer[0] += 48;	// 앞으로 48, 정지 32, 뒤로 16
		buffer[0] += vector_real / 256;
		buffer[1] = vector_real - ( vector_real / 256 ) * 256;
